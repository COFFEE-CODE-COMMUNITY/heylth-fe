...
import {
  addSleepTracker,
  getAllSleepTracker,
} from "../services/sleepTrackerService";
...
export const InputData = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [sleepData, eatData, screenData] = await Promise.all([
          getAllSleepTracker(),
          getAllEatTracker(),
          getAllScreenTime(),
        ]);
  ...
  const saveSleepData = async () => {
    try {
      ...
      const errMessage =
        error?.response?.data?.error || "Error saving sleep data!";
  ...
  const saveMealsData = async () => {
    try {
      ...
      const errMessage =
        error?.response?.data?.error || "Error saving meal data!";
      setMessage(errMessage);
      setMessageType("error");
      setTimeout(() => {
        setMessage("");
